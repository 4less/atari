function bubbleChart (myDivId, series, data, categories, width, height, xlabel="xlabel", ylabel="ylabel", zlabel="zlabel", catlabel="catlabel") {
	...
	returnDictionary["init"] = function() {
		...
		var data = d3.zip(series, data, categories);
...
function setLegend(chartGroup, colorDict, chartWidth, zData, zScale) {
	...
	legend.append("text")
		.attr("x", 0)
		.attr("y", fontSizeHeading)
		.attr("font-size", fontSizeHeading)
		.attr("font-weight", "bold")
		.text(catLabel);

	l.append("circle")
		.attr("cx", 0)
		.attr("cy", function(data, i) { return i*lineHeight + 2/3*fontSize + fontSizeHeading*1.5; })
		.attr("r", radius)
		.attr("fill-opacity", function(data) { return 0.5;})
		.attr("fill", function(data) { return data.value; });
	l.append("text")
		.attr("x", 2* radius + 4)
		.attr("y", function(data, i) { lastIndex = i; return i*lineHeight + fontSize + fontSizeHeading*1.5; })
		.attr("font-size", fontSize)
		.text(function(data) { console.log(data.key); return data.key;});

	legend.append("circle")
		.attr("cx", 0)
		.attr("cy", (lastIndex+3)*lineHeight- radius -1 + fontSizeHeading*1.5)
	...
	legend.append("text")
		...
		.attr("y", (lastIndex+3) * lineHeight + fontSizeHeading*1.5)
		.text(d3.min(zData));

	legend.append("circle")
		.attr("cx", 0)
		.attr("cy", (lastIndex+4)*lineHeight + zScale(d3.max(zData)) - radius -1 + fontSizeHeading*1.5)
	...
	legend.append("text")
		...
		.attr("y", (lastIndex+4) * lineHeight + zScale(d3.max(zData)) + fontSizeHeading*1.5)
		.text(d3.max(zData));
